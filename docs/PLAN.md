...
> **ความคืบหน้า:** M0–M10 เสร็จ · **M11** ออก [`v0.1.0-beta.1`](https://github.com/maxsrisupan/MinkQuickLax/releases/tag/v0.1.0-beta.1) แล้ว (2026-09-15) · ถัดไป: ผู้ใช้ลองใช้รุ่นทดลองอย่างน้อย 1 สัปดาห์ + รายการตรวจหัวข้อ 8.2 → ทดสอบอัปเดตด้วย beta.2 → ออก `v0.1.0` (หัวข้อ 6)
...
- **ยังต้องตรวจ:** ภาพตอนกดเปิด (ไม่ได้กดเปิด link จริงระหว่างทดสอบเพื่อไม่ให้เปิดโปรแกรมอื่นบนเครื่องผู้ใช้) · กล่องแจ้งเตือน แผงกลุ่ม หน้าสแกนใน HUD/Dot Matrix หลังเปลี่ยนไอคอน (ตรวจแผ่นพื้นแล้วใน M7) · ลดภาพเคลื่อนไหว · High contrast · จอ scale 125–150%
...
- [ ] ทดสอบใน Windows Sandbox ตามหัวข้อ 8.2 (เครื่องนี้ไม่มี Sandbox · ติดตั้ง/ถอนบนเครื่องจริงผ่านแล้ว ดูผลตรวจด้านล่าง)
- [ ] ออก `v0.1.0-beta.1` ✔ (2026-09-15) → ใช้เองอย่างน้อย 1 สัปดาห์ → `v0.1.0`
...
- **ยังต้องทำ/ตรวจ:** อัปเดตจาก beta.1 → beta.2 จาก GitHub จริง (ต้องออก beta.2) · รายการในหัวข้อ 8.2 บนเครื่องจริง · ใช้เองอย่างน้อย 1 สัปดาห์ก่อน `v0.1.0`
...
- [ ] ติดตั้งบนเครื่องใหม่ ไม่มี UAC · ผ่านหน้า SmartScreen ตามคู่มือได้ (ไม่มี UAC ตรวจแล้วบนเครื่องนักพัฒนา · SmartScreen ต้องดาวน์โหลดจากหน้า Release บนเครื่องอื่น)
...
| 2026-09-15 | ภาพโฮโลแกรม (HUD) และขาวดำ (Dot Matrix) | สร้างจากภาพ icon ครั้งเดียวเป็น bitmap 128px เก็บในหน่วยความจำคู่กับภาพต้นฉบับ (`ConditionalWeakTable`) แล้ววางซ้อนภาพจริงโดยไล่ความทึบตาม t · ไม่เขียนลง icon cache บนดิสก์ · ไม่ใช้ `ShaderEffect` | สร้างเร็วพอ (30 ภาพ) และหายไปเองเมื่อภาพถูกทิ้ง · ไฟล์ใน cache จะต้องตามลบเมื่อสูตรสีเปลี่ยน · หน้าต่างไอคอนวาดด้วย CPU ซึ่ง `ShaderEffect` ไม่ทำงาน |
...
| 2026-09-15 | ความใกล้ t | `ProximityAnimator` ไล่ค่า t ไปพร้อมความทึบและขนาด แล้วส่งให้ `IconWindow.SetProximity` · โหมดแก้ไขใช้ t = 1 | ใช้ timer 33ms ตัวเดิม ไม่เพิ่ม animation ต่อไอคอน |
